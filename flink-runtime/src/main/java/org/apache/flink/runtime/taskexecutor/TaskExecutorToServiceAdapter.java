...
public class TaskExecutorToServiceAdapter implements TaskManagerRunner.TaskExecutorService {
    ...
    @Override
    public void start() {
        /** 启动taskExecute 给自己发个start消息
         * taskExecutor是rpcEndpoint,那么在调用start方法前，会调用rpcEndpoint的onStart生命周期方法
         * 同理，调用stop方法前也会调用rpcEndpoint的onStop生命周期方法
         * */
        taskExecutor.start();
    }
    ...
    /**
     * 调用closeAsync方法前也会调用rpcEndpoint的onStop生命周期方法
     * @return
     */
    @Override
