...
/**
 * 基于分布式系统的抽象高可用服务（例如 Zookeeper、Kubernetes）。它将有助于创建所有领导者选举检索服务和清理。 请在
 * getLeaderPathForResourceManager、getLeaderPathForDispatcher、getLeaderPathForJobManager、
 * getLeaderPathForRestServer 的实现中返回正确的领导者名称。返回的 leader 名称是 Kubernetes 中的 ConfigMap 名称和 Zookeeper
 * 中的子路径。应实施 close() 和 closeAndCleanupAllData() 以销毁资源。抽象类还负责确定应该重用哪个组件服务。 例如，runningJobsRegistry
 * 只创建一次，可以重复使用很多次。
 */
