...
/**
 * HighAvailabilityServices 允许访问高可用性设置所需的所有服务。特别是，这些服务提供对高可用性存储和注册表的访问， 以及分布式计数器和领导者选举。
 * ResourceManager 领导选举和领导检索 JobManager 领导选举和领导检索 检查点元数据的持久性 注册最新完成的检查点 BLOB 存储的持久性 标记作业状态的注册表 RPC
 * 端点的命名
 */
